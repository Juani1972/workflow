...
# --- actions added on 29 Aug 2026 (get_meeting_load, get_attendance,
# --- mark_absent, reassign_booking) ---

@patch("calcom_pro_handler.requests.get")
def test_get_meeting_load_sums_hours_and_breaks_down_by_day(mock_get):
    mock_get.return_value = _mock_response({
        "data": [
            {"start": "2026-03-02T09:00:00Z", "duration": 60},
            {"start": "2026-03-02T14:00:00Z", "duration": 30},
            {"start": "2026-03-03T10:00:00Z", "duration": 120},
        ]
    })
    out = handler.get_meeting_load({"from_date": "2026-03-02", "to_date": "2026-03-08"}, {})
    assert out["total_hours"] == 3.5           # 1h + 0.5h + 2h
    assert out["hours_by_day"]["2026-03-02"] == 1.5
    assert out["hours_by_day"]["2026-03-03"] == 2.0
    assert out["booking_count"] == 3


@patch("calcom_pro_handler.requests.get")
def test_get_meeting_load_falls_back_to_start_end_when_no_duration(mock_get):
    mock_get.return_value = _mock_response({
        "data": [{"start": "2026-03-02T09:00:00Z", "end": "2026-03-02T10:30:00Z"}]
    })
    out = handler.get_meeting_load({"from_date": "2026-03-02", "to_date": "2026-03-08"}, {})
    assert out["total_hours"] == 1.5


@patch("calcom_pro_handler.requests.get")
def test_get_meeting_load_handles_empty_range(mock_get):
...
@patch("calcom_pro_handler.requests.get")
def test_get_attendance_splits_present_and_absent(mock_get):
    mock_get.return_value = _mock_response({
        "data": {
            "attendees": [
                {"email": "a@x.com", "absent": False},
                {"email": "b@x.com", "absent": True},
                {"email": "c@x.com", "noShow": True},
            ],
            "hostNoShow": False,
        }
    })
    out = handler.get_attendance({"booking_uid": "bk_1"}, {})
    assert mock_get.call_args.args[0] == "https://api.cal.com/v2/bookings/bk_1"
    assert out["present"] == ["a@x.com"]
    assert sorted(out["absent"]) == ["b@x.com", "c@x.com"]
    assert out["host_absent"] is False


@patch("calcom_pro_handler.requests.post")
def test_mark_absent_hits_mark_absent_endpoint(mock_post):
    mock_post.return_value = _mock_response({"status": "success"})
    handler.mark_absent(
        {"booking_uid": "bk_1", "host_absent": False, "absent_attendee_emails": ["b@x.com"]}, {}
    )
    assert mock_post.call_args.args[0] == "https://api.cal.com/v2/bookings/bk_1/mark-absent"
    body = mock_post.call_args.kwargs["json"]
    assert body["host"] is False
    assert body["attendees"] == [{"email": "b@x.com", "absent": True}]


@patch("calcom_pro_handler.requests.post")
def test_reassign_booking_hits_reassign_endpoint(mock_post):
    mock_post.return_value = _mock_response({"status": "success"})
    handler.reassign_booking({"booking_uid": "bk_1", "target_user_id": "42"}, {})
    assert mock_post.call_args.args[0] == "https://api.cal.com/v2/bookings/bk_1/reassign/42"
